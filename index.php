...
<!-- <?php  
    ...
    if(!$conn)  
        ...
        echo "Se pudo conectar satisfactoriamente a la base de datos";  
    $query = "CREATE DATABASE curso";  
    if (mysqli_query($conn, $query)) {  
        echo "Se pudo conectar satisfactoriamente a la base de datos curso";  
    } else {  
...
<!-- CONEXION A BASE DE DATOS, CONSULTA DE ALUMNOS Y MUESTRA DE 1 -->
<!-- <?php
    $servername = "localhost:3307";  
    $username = "root";
    $password = "";
    ...
    $sentencia = $mysqli->prepare("SELECT idalumno, nombre FROM alumno");
    $sentencia->execute();
    $resultado = $sentencia->get_result();
    $fila = $resultado->fetch_assoc();

    printf("id = %s (%s)\n", $fila['idalumno'], gettype($fila['idalumno']));
...
<!-- CONEXION A BASE DE DATOS, CONSULTA DE ALUMNOS Y MUESTRA DE 1 -->
 <!-- <?php
    ...
    while ($fila) {
        printf("idalumno = %s (%s), nombre = %s (%s)\n", $fila['idalumno'], gettype($fila['idalumno']), $fila['nombre'], gettype($fila['nombre']));
        printf("<br></br>");
        $fila = $resultado->fetch_assoc();
    }
?>   -->